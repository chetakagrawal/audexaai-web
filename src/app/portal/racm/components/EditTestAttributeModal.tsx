...
import { TestAttribute, TestAttributeCreate } from '@/lib/api';

interface EditTestAttributeModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export default function EditTestAttributeModal({
  isOpen,
  onClose,
  onSubmit,
  testAttribute,
  ...
  // Update form data when testAttribute changes
  useEffect(() => {
    if (testAttribute) {
      setFormData({
        code: testAttribute.code,
        name: testAttribute.name,
        frequency: testAttribute.frequency,
        test_procedure: testAttribute.test_procedure,
        expected_evidence: testAttribute.expected_evidence,
      });
    }
  }, [testAttribute]);
  ...
  if (!isOpen || !testAttribute) return null;

  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <h2 className="text-2xl font-bold text-gray-900 mb-2">Edit Test Attribute</h2>
